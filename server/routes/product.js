...
router.post("/products", async (req, res) => {
  try {
    ...
    if (!title || !price || !description || !category || isNaN(price)) {
      return res
        .status(400)
        .json("Please fill in all required information correctly.");
    }
    ...
    const NewItem = `
     ...
     RETURNING *;
   `;
    const fromatRatings = rating
      ? typeof rating === "object"
        ? JSON.stringify(rating)
        : rating
      : null;
    const Result = await Pool.query(NewItem, [
      title,
      price,
      category,
      description,
...
router.put("/products/:id", async (req, res) => {
  ...
  try {
    ...
    const Result = await Pool.query(
      "SELECT id,title,price,category,description,image,rating  FROM products WHERE id=$1",
      [id]
    );
